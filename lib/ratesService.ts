          ...
          if (data && data.length > 0) {
            ...
            if (!isNaN(buyRate) && !isNaN(sellRate)) {
              rates.push({
                ...
                buy: buyRate * 100, // Convert to "per 100" format (12,500 -> 1,250,000 per 100 USD)
                sell: sellRate * 100, // Convert to "per 100" format (12,570 -> 1,257,000 per 100 USD)
      ...
      rates.push(
        {
          bank: 'Universal Bank',
          currency: 'USD',
          buy: 1245000, // Convert to "per 100" format (12,450 -> 1,245,000 per 100 USD)
          sell: 1265000, // Convert to "per 100" format (12,650 -> 1,265,000 per 100 USD)
          date: today,
        },
        {
          bank: 'Universal Bank',
          currency: 'EUR',
          buy: 1300000, // Convert to "per 100" format (13,000 -> 1,300,000 per 100 EUR)
          sell: 1520000, // Convert to "per 100" format (15,200 -> 1,520,000 per 100 EUR)
          date: today,
        },
        {
          bank: 'Universal Bank',
          currency: 'RUB',
          buy: 15000, // Convert to "per 100" format (150 -> 15,000 per 100 RUB)
          sell: 16100, // Convert to "per 100" format (161 -> 16,100 per 100 RUB)
          date: today,
        }
      );
    }
...
/**
 * Fetches exchange rates from NBU via API route
 * API: /api/nbu-rates
 * Uses: Server-side scraping with regex parsing (handled by the API route)
 */
async function fetchNbuRates(): Promise<ExchangeRate[]> {
  try {
    ...
    if (data.success && data.rates) {
      console.log(`NBU rates fetched: ${data.rates.length}`);
      return data.rates;
    } else {
      console.log('NBU API returned fallback values');
      return data.rates || [];
    }
  } catch (error) {
...
/**
 * Fetches exchange rates from InfinBank via API route
 * API: /api/infinbank-rates
 * Uses: Server-side scraping with regex parsing (handled by the API route)
 */
async function fetchInfinBankRates(): Promise<ExchangeRate[]> {
  try {
    const response = await fetch('/api/infinbank-rates', {
      headers: {
        Accept: 'application/json',
      },
    });
    ...
    if (!response.ok) {
      throw new Error(`InfinBank API error: ${response.status}`);
    }

    const data = await response.json();

    if (data.success && data.rates) {
      console.log(`InfinBank rates fetched: ${data.rates.length}`);
      return data.rates;
    } else {
      console.log('InfinBank API returned fallback values');
      return data.rates || [];
    }
  } catch (error) {
